...
export async function predictPlantDiseaseWithGemini(imageBuffer, mimeType) {
    ...
    console.log(`[Gemini Multimodal] Base64 length: ${base64Data.length}, MIME: ${cleanMimeType}`);
    ...
    const prompt = [
        ...
        '{"plant":"PlantName","disease":"DiseaseName","confidence":75,"symptoms":"symptoms","cause":"cause","explanation":"brief explanation"}',
        "Rules:",
        "- plant: one of Tomato, Apple, Grape, Peach, Pepper, Potato, Strawberry, Cherry, Corn, Blueberry, Orange, Raspberry, Soybean, Squash, or best guess",
        '- disease: exact name (e.g. "Early blight") or "healthy" if plant is healthy',
    ...
    // Extract JSON - strip markdown code fences if model wraps response
    let jsonStr = content;
    ...
    console.log("[Gemini Multimodal] Parsed:", parsed.plant, parsed.disease, parsed.confidence);
    return {
